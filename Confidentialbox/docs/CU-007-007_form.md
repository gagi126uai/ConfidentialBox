...
## Diagrama de secuencia (CU-007-007)
```plantuml
@startuml
title CU-007-007 - Secuencia de proyección en AI Security Dashboard
actor "Analista de seguridad" as Analyst
participant "AI Security Dashboard" as Dashboard
participant "Security API" as API
participant "AISecurityService" as SecuritySvc
database "ThreatScores" as Scores
database "RiskScores" as Riesgos
database "Alertas" as Alertas

Analyst -> Dashboard : Abrir panel
Dashboard -> API : Solicitar KPIs y recomendaciones
API -> SecuritySvc : Pedir agregados Threat/Risk/Alertas
SecuritySvc -> Scores : Consultar ThreatScores
SecuritySvc -> Riesgos : Leer RiskScores por usuario
SecuritySvc -> Alertas : Obtener alertas recientes
SecuritySvc --> API : Dataset consolidado
API --> Dashboard : Responder KPIs + acciones sugeridas
Dashboard --> Analyst : Mostrar gráficos, tablas y enlaces
@enduml
```
